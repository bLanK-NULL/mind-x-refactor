...
describe('NodeRenderer', () => {
  ...
  it('keeps topic inline editing inside TopicNodeContent', () => {
    ...
    expect(source).toContain('localTitle')
    ...
    expect(source).not.toContain('@blur="commitEdit"')
    expect(source).not.toContain('@keydown.enter.prevent="commitEdit"')
  })
  ...
  it('keeps task item interactions inside TaskNodeContent instead of the inspector pane', () => {
    ...
    expect(taskContentSource).toContain('function localTitleFor')
    expect(taskContentSource).toContain('function buildItemsFromLocal')
    ...
    expect(taskContentSource).toContain('function onTaskInput')
    ...
    expect(taskContentSource).toContain('...buildItemsFromLocal(),')
    expect(taskContentSource).toContain('commitItems(buildItemsFromLocal().filter')
    expect(taskContentSource).toContain('commitItems(buildItemsFromLocal().map((item) =>')
    expect(taskContentSource).toContain('commitItems(buildItemsFromLocal().map((candidate) =>')
    ...
    expect(taskContentSource).toContain(':aria-label="`Mark ${localTitles[item.id] ?? item.title} complete`"')
    expect(taskContentSource).toContain(':aria-label="`Edit task ${localTitles[item.id] ?? item.title}`"')
    expect(taskContentSource).toContain(':aria-label="`Delete task ${localTitles[item.id] ?? item.title}`"')
    expect(taskContentSource).toContain('@pointerdown.stop')
    expect(taskContentSource).toContain('@input="onTaskInput(item)"')
    expect(taskContentSource).toContain('@keydown.esc.prevent')
    expect(taskContentSource).not.toContain('@blur="commitTaskTitle"')
    expect(taskContentSource).not.toContain('@keydown.enter.prevent')
  ...
  it('keeps attachment content read-only while code and link own their own interactions', () => {
    ...
    expect(readNodeContentSource('CodeNodeContent')).toContain("emit('commit', { code: value })")
    expect(readNodeContentSource('CodeNodeContent')).toContain('@change="onCodeChange"')
  ...
  it('renders code nodes as an always-editable CodeMirror surface', () => {
    ...
    expect(source).toContain('const localCode = ref(props.node.data.code)')
    ...
    expect(source).toContain('v-model="localCode"')
    ...
    expect(source).not.toContain('@blur="commitCode"')
  })
